...
The compiler uses the builtin function definitions themselves as code-generation templates in most
cases. Special forms (functions with unevaluated arguments), require a small code-generatin hook.
...
for instance, basically generates a "PHI" node.
(You can quibble whether this is "really" SSA or not--I've written compilers using LLVM and I think it
is--nevertheless it's useful and clarifying to think about it that way, and as a naming scheme, labeling variables that contain the names of _values_ "ssa" has been absolutely indispensible.)
The result is JavaScript in 1-1 correspondence
between Scheme and the JavaScript you'd write yourself, except for a bunch of assignments
to SSA variables. JavaScript JITs will certainly erase those variables.
